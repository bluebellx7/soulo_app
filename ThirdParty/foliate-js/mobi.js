...
export class MOBI extends PDB {
    ...
    async open(file) {
        ...
        await this.#setup()
        if (this.headers.palmdoc.numTextRecords > 10000) throw new Error('Book exceeds processing limit')
        const first = await this.loadText(0)
        if (new TextDecoder().decode(first.slice(0, 4)) === '%MOP') {
            const parts = [first]; let total = first.length
            for (let i = 1; i < this.headers.palmdoc.numTextRecords; i++) {
                const part = await this.loadText(i); total += part.length
                if (total > 134217728) throw new Error('Book exceeds processing limit')
                parts.push(part)
            }
            const bytes = new Uint8Array(total); let cursor = 0
            for (const part of parts) { bytes.set(part, cursor); cursor += part.length }
            const header = new DataView(bytes.buffer)
            const offset = header.getUint32(12), length = header.getUint32(16)
            if (offset < 20 || offset + length > total) throw new Error('Damaged Print Replica book')
            return { souloPDF: bytes.slice(offset, offset + length) }
        }
        return isKF8 ? new KF8(this).init() : new MOBI6(this).init()
    }
    #getHeaders(buf) {
        const palmdoc = getStruct(PALMDOC_HEADER, buf)
        if (palmdoc.encryption) throw new Error("DRM-protected book. Import an unprotected copy.")
...
class MOBI6 {
    ...
    async init() {
        ...
        for (let i = 0; i < this.mobi.headers.palmdoc.numTextRecords; i++) {
            ...
            if (recordBuffers.reduce((n,b) => n + b.byteLength, 0) > 134217728) throw new Error("Book exceeds processing limit")
        }
